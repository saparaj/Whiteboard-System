...
class WhiteboardSession extends Thread{
	...
	public void run(){
		try {
			...
			archive = new ChildDataObject(temp.getArray());
			...
			updateUserList();
		 	...
		 	for(;;){
				...
				receiver((ChildDataObject)ois.readObject());
			}

		} catch(Exception e){
			try {
				log("Sending leave message");
				ChildDataObject packet = new ChildDataObject(name + " has exited chat.\n");
				broadcaster(packet);
	...
	public void receiver(ChildDataObject packet){
		log("Parsing packet");
		try {
	...
	public void updateUserList(){
		...
		ChildDataObject packet = new ChildDataObject(userList);
	...
	public synchronized void broadcaster(ChildDataObject packet) throws IOException {
	...
	public void updateArchive(ChildDataObject packet){
